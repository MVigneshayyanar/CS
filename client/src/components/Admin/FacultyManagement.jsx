    ...
    <div className="space-y-8 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 mt-2">Add and view faculty from backend database</p>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm font-semibold text-slate-700 uppercase tracking-wider">Specialization</th>
            ...
            <tbody className="divide-y divide-slate-200">
              {isLoading && (
                <tr>
                  <td colSpan="6" className="px-6 py-6 text-center text-slate-500">Loading faculty...</td>
                </tr>
              )}
              {!isLoading && filteredFaculty.map((member) => (
                <tr key={member.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-slate-700 font-mono">{member.empId}</td>
                  <td className="px-6 py-4">
                    <span className="bg-teal-600/20 text-teal-300 px-2 py-1 rounded text-sm">
                      {member.department}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-slate-700">{member.specialization}</td>
                  <td className="px-6 py-4">
              ...
              {!isLoading && filteredFaculty.length === 0 && (
                <tr>
                  <td colSpan="6" className="px-6 py-6 text-center text-slate-400">No faculty found.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      {showModal && (
        <Modal title={editingFaculty ? 'Edit Faculty' : 'Add Faculty'} onSubmit={handleSubmitFaculty} onClose={closeModal} isSubmitting={isSubmitting}>
          <div className="grid md:grid-cols-2 gap-4">
            <input type="text" placeholder="Full Name" value={facultyForm.name} onChange={(e) => setFacultyForm((prev) => ({ ...prev, name: e.target.value }))} className="p-4 bg-slate-50 border border-slate-200 rounded-lg text-slate-900" required />
            <input type="email" placeholder="Email" value={facultyForm.email} onChange={(e) => setFacultyForm((prev) => ({ ...prev, email: e.target.value }))} className="p-4 bg-slate-50 border border-slate-200 rounded-lg text-slate-900" required />
            <input type="text" placeholder="ID" value={facultyForm.empId} onChange={(e) => setFacultyForm((prev) => ({ ...prev, empId: e.target.value }))} className="p-4 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 md:col-span-2" required />
          </div>
        </Modal>
      )}
    </div>
