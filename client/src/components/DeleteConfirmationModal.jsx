...
const DeleteConfirmationModal = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6">
      <div className="w-full max-w-md rounded-2xl border border-theme bg-card shadow-xl">
        <div className="p-5 sm:p-6">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <h3 className="text-base sm:text-lg font-bold text-heading">{title}</h3>
              <p className="mt-1 text-sm text-body">{message}</p>
              {itemName ? (
                <p className="mt-2 rounded-lg bg-alt border border-theme px-3 py-2 text-sm font-semibold text-heading break-words">
                  {itemName}
                </p>
              ) : null}
            </div>
          ...
          <div className="mt-6 flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
            <button
              ...
              className="px-4 py-2 rounded-lg border border-theme bg-alt text-heading hover:bg-alt transition-colors disabled:opacity-60"
            >
              {cancelLabel}
            </button>
            <button
