...
const SectionHeader = ({
  ...
  <div className="flex items-center justify-between mb-4">
    ...
    <div className="flex items-center gap-2">
      {badge && (
        <span
          className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${badgeBg} ${badgeColor}`}
        >
          {badge}
        </span>
      )}
...
const FacultyLabManagement = () => {
  ...
  const classData =
    (selectedClass?.students || 0) > 0
      ? Array.from({ length: Math.min(selectedClass.students, 10) }).map(
          (_, i) => ({
            id: `${selectedClass.name}-${i + 1}`,
            name: `Student ${i + 1}`,
            experiments: Array.from(
              { length: Math.max(10, experiments.length || 10) },
              (_, ei) =>
                ei < Math.round((selectedClass.completionRate || 0) / 10),
            ),
          }),
        )
      : [];
          ...
          <>
            ...
            {currentView === "experiments" && selectedClass && (
              <>
                <SectionHeader
                  icon={FlaskConical}
                  title={`Experiments — ${selectedClass.name}`}
                  badge={`${experiments.length} Experiments`}
                  action={
                    <button
                      onClick={() => setShowDataSheet(true)}
                      className="flex items-center gap-2 px-3 py-2 bg-emerald-600 text-white text-xs font-bold rounded-xl hover:bg-emerald-700 transition-all shadow-sm"
                    >
                      <svg
                ...
                {experiments.length === 0 ? (
                  <p className="text-sm text-slate-400 text-center py-10">
                    No experiments found for this class.
                  </p>
                ) : (
