      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-16 md:pt-8 pb-10 sm:pb-12">
        ...
        <div className="relative bg-[#1a6b5c] rounded-2xl px-4 sm:px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between overflow-hidden gap-4 mb-6">
          ...
          <button
            ...
            className="relative z-10 bg-card text-[#134d42] px-5 py-2.5 rounded-xl hover:bg-[#f0f7f5] transition-all flex items-center justify-center shadow-sm text-sm font-bold w-full sm:w-auto"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
          <div className="bg-card border border-theme rounded-2xl p-4 shadow-sm">
            <p className="text-[11px] text-muted font-bold uppercase tracking-wider mb-1">Total Experiments</p>
            <p className="text-2xl font-extrabold text-heading">{lab?.experiments?.length || 0}</p>
            <p className="text-xs text-body mt-1">Published problems</p>
          </div>
          <div className="bg-card border border-theme rounded-2xl p-4 shadow-sm">
            <p className="text-[11px] text-muted font-bold uppercase tracking-wider mb-1">Test Scale</p>
            <p className="text-2xl font-extrabold text-heading">
              {(lab?.experiments || []).reduce((acc, curr) => acc + (curr.testCases?.length || 0), 0)}
            </p>
            <p className="text-xs text-body mt-1">Total test cases across lab</p>
          </div>
          <div className="bg-card border border-theme rounded-2xl p-4 shadow-sm">
            <p className="text-[11px] text-muted font-bold uppercase tracking-wider mb-1">Language</p>
            <p className="text-2xl font-extrabold text-heading">{lab?.language || 'N/A'}</p>
            <p className="text-xs text-body mt-1">Primary runtime environment</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-heading flex items-center gap-2">
            <Code className="w-5 h-5 text-[#1a6b5c]" />
            Lab Experiments
          </h2>
        ...
        <div className="grid gap-4">
          {lab?.experiments?.length === 0 ? (
            <div className="bg-card border-2 border-dashed border-theme rounded-2xl p-16 text-center">
              <Code className="w-12 h-12 text-muted mx-auto mb-4" />
              <h3 className="text-lg font-bold text-heading">No experiments yet</h3>
              <p className="text-body text-sm max-w-xs mx-auto mt-2">
                Click the button above to add your first coding experiment for this laboratory.
              </p>
            </div>
          ) : (
            lab.experiments.map((exp, index) => (
              <div key={index} className="bg-card border border-theme rounded-2xl p-5 shadow-sm hover:shadow-md transition-all group">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-8 h-8 bg-[#f0f7f5] rounded-lg flex items-center justify-center">
                        <span className="text-[#134d42] font-bold text-xs">Ex {index + 1}</span>
                      </div>
                      <h3 className="text-lg font-bold text-heading">{exp.title}</h3>
                    </div>
                    <p className="text-sm text-body leading-relaxed max-w-2xl">{exp.description || 'No description provided.'}</p>
                    
                    <div className="flex flex-wrap items-center gap-4 mt-4">
                      <div className="flex items-center gap-1.5 text-xs font-semibold text-muted bg-alt px-2.5 py-1 rounded-full border border-theme-light">
                  ...
                  <div className="flex flex-col gap-2">
                    <button 
                      ...
                      className="p-2 text-muted hover:text-blue-500 hover:bg-blue-50 rounded-xl transition-all"
        ...
        {showAddModal && (
            ...
            footer={
              <>
                <button 
                  ...
                  className="px-6 py-2 text-body bg-alt border border-theme rounded-lg hover:bg-alt transition-colors w-full sm:w-auto font-semibold"
                >
                  Cancel
                </button>
                <button 
                  ...
                  className="px-6 py-2 bg-[#1a6b5c] text-white rounded-lg hover:bg-[#134d42] transition-all flex items-center justify-center shadow-lg shadow-[#2a8c78]/20 w-full sm:w-auto disabled:opacity-60 gap-2 font-bold"
                >
          ...
          >
            <div className="space-y-6">
              <div>
                <label className="block text-[11px] font-bold text-muted uppercase tracking-widest mb-2 font-mono">Title</label>
                <input
                  type="text"
                  ...
                  className="w-full p-4 bg-alt border border-theme rounded-xl text-heading placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#2a8c78] focus:border-[#2a8c78] transition-all font-medium"
                />
              </div>

              <div>
                <label className="block text-[11px] font-bold text-muted uppercase tracking-widest mb-2 font-mono">Description</label>
                <textarea
                  ...
                  className="w-full p-4 bg-alt border border-theme rounded-xl text-heading placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#2a8c78] focus:border-[#2a8c78] transition-all"
                  rows="3"
                />
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-4">
                  <label className="text-[11px] font-bold text-muted uppercase tracking-widest font-mono">Input / Output Test Cases</label>
                  <button 
                    type="button" 
                    onClick={addTestCase} 
                    className="text-[#1a6b5c] hover:text-[#134d42] text-xs font-bold bg-[#f0f7f5] px-3 py-1.5 rounded-lg transition-all"
                  >
                    + Add New Case
                  </button>
                </div>
                <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
                  {experimentForm.testCases.map((tc, i) => (
                    <div key={i} className="flex gap-3 items-center bg-card p-3 border border-theme-light rounded-xl shadow-sm">
                      <div className="flex-1 space-y-2">
                        <textarea
                          placeholder="Standard Input"
                          value={tc.input}
                          onChange={(e) => updateTestCase(i, 'input', e.target.value)}
                          className="w-full p-2.5 bg-alt border border-theme rounded-lg text-sm text-heading placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#2a8c78] transition-all font-mono"
                          rows="2"
                        />
                        <textarea
                      ...
                      {experimentForm.testCases.length > 1 && (
                        <button 
                          ...
                          className="text-muted hover:text-red-500 p-2 bg-alt rounded-lg hover:bg-red-50 transition-all"
                        >
                          <X className="w-4 h-4" />
                        </button>
                      )}
