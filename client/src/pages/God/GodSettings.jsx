...
const Field = ({ label, value }) => (
  <div>
    ...
    <div className="px-3 py-2 bg-alt border border-theme-light rounded-xl text-body text-xs font-semibold">
      {value}
    </div>
  </div>
);
...
const ToggleRow = ({ label, description, checked, onChange }) => (
  <div className="flex items-center justify-between py-3 border-b border-theme-light last:border-b-0">
    <div>
    ...
    <button
      ...
      className={`relative w-9 h-5 rounded-full transition-all duration-200 ${checked ? 'bg-[#2a8c78]' : 'bg-alt'}`}
    >
      <span
...
const PwField = ({ label, show, onToggle, value, onChange, placeholder, hint }) => (
  <div>
    ...
    {hint && <p className="text-[10px] text-muted mt-1 font-medium">{hint}</p>}
  </div>
);
...
const GodSettings = () => {
  ...
  const strengthColors = ['bg-alt', 'bg-red-400', 'bg-amber-400', 'bg-[#3aa892]', 'bg-[#1a6b5c]'];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-16 md:pt-8 pb-10 sm:pb-12">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-extrabold text-heading leading-tight">God Settings</h1>
              <p className="text-xs text-muted">Manage your profile and preferences</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col xl:flex-row gap-5 items-start">
          <div className="w-full xl:w-56 flex-shrink-0 flex flex-col gap-4 xl:sticky xl:top-8">
            <div className="bg-card rounded-2xl border border-theme-light shadow-sm overflow-hidden">
              ...
              <div className="px-5 pb-5 text-center">
                ...
                <p className="text-sm font-extrabold text-heading">{adminName}</p>
                <p className="text-[11px] text-[#1a6b5c] font-bold mt-0.5">{adminEmail}</p>
                <p className="text-[10.5px] text-muted mt-0.5">Platform Governance</p>
            ...
            <nav className="bg-card rounded-2xl border border-theme-light shadow-sm p-2.5 overflow-x-auto">
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-4 w-full">
            <div className="relative bg-[#1a6b5c] rounded-2xl px-4 sm:px-6 py-5 flex items-start sm:items-center justify-between overflow-hidden gap-4">
              ...
              <SectionCard icon={Shield} title="Administrative Security">
                <div className="pt-1">
                  <div className="flex items-center gap-2 mb-3">
                    <Monitor className="w-4 h-4 text-[#1a6b5c]" />
                    <h4 className="text-xs font-extrabold text-heading uppercase tracking-wider">
                      Sessions
                    </h4>
                  </div>

                  <div className="bg-alt border border-theme rounded-xl p-3 mb-3">
                    <p className="text-xs font-bold text-heading mb-1">Current Session</p>
            ...
            {activeSection === 'add-god' && (
              <SectionCard icon={UserPlus} title="Create God Account">
                {godCredentials && (
                  <div className="mb-4 bg-[#f0f7f5] border border-[#c2e6de] rounded-xl p-3 text-teal-900">
                ...
                <form onSubmit={handleAddGod} className="space-y-4">
                  <div className="grid grid-cols-1 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2.5 bg-alt border border-theme rounded-xl text-xs text-heading focus:outline-none focus:ring-2 focus:ring-teal-200 focus:border-[#3aa892] transition-all"
                      />
                    </div>
                    <div>
                      <label className="block text-[9.5px] font-extrabold text-muted uppercase tracking-widest mb-1.5">
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 px-5 py-2.5 bg-[#1a6b5c] text-white text-xs font-extrabold rounded-xl hover:bg-[#134d42] disabled:opacity-50 transition-all shadow-sm shadow-[#2a8c78]/20 w-full sm:w-auto"
                  >
                ...
                <div className="mt-6 pt-5 border-t border-theme-light">
                  <h4 className="text-xs font-extrabold text-heading uppercase tracking-wider mb-3">
                    Existing God Accounts
                  </h4>

                  {loadingGodUsers ? (
                    <p className="text-xs text-muted">Loading God accounts...</p>
                  ) : godUsers.length === 0 ? (
                    <p className="text-xs text-muted">No God accounts found.</p>
                  ) : (
                    <div className="space-y-2">
                      {godUsers.map((user) => {
                        ...
                        return (
                          <div
                            key={user.id}
                            className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-alt border border-theme rounded-xl px-3 py-2.5"
                          >
                            <div>
                              <p className="text-xs font-bold text-heading">{user.username}</p>
                              <p className="text-[11px] text-body">{user.email || 'No email'}</p>
                            </div>
                            <button
                              type="button"
